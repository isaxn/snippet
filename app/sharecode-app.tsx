...
export default function ShareCodeApp({ user }: { user: User }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (response.status === 401) { setUploading(false); setAuthOpen(true); return; }
  ...
  async function submitAuth(event: React.FormEvent<HTMLFormElement>) {
    ...
    const data = await response.json();
    setAuthBusy(false);
    if (!response.ok) { setToast(data.error || "Autentikasi gagal"); return; }
  ...
  return <main>
    <header className="topbar">
      ...
      <div className="account">
        {user ? <><span className="userPill">{user.name.charAt(0).toUpperCase()} <b>{user.name.split(" ")[0]}</b></span><button className="ghost small" onClick={logout}>Keluar</button></> : <button className="ghost" onClick={() => setAuthOpen(true)}>Masuk / Buat akun</button>}
        <button className="primary" onClick={() => user ? setUploading(true) : setAuthOpen(true)}>+ Upload kode</button>
      </div>
    </header>
    ...
    <section className="hero">
      <div className="heroCopy">
        ...
        <div className="heroActions"><button className="primary large" onClick={() => user ? setUploading(true) : setAuthOpen(true)}>Bagikan kode kamu →</button><a className="textLink" href="#jelajah">Jelajahi snippet ↓</a></div>
    ...
    {authOpen && <div className="overlay" onMouseDown={() => setAuthOpen(false)}><form className="modal authModal" onSubmit={submitAuth} onMouseDown={e => e.stopPropagation()}><button type="button" className="close" onClick={() => setAuthOpen(false)}>×</button><span className="kicker">AKUN SHARECODE</span><h2>{authMode === "login" ? "Selamat datang kembali" : "Buat akun baru"}</h2><p className="authIntro">{authMode === "login" ? "Masuk untuk mengunggah dan membagikan kode." : "Daftar gratis, lalu mulai publikasikan snippet kamu."}</p>{authMode === "register" && <label>Nama<input name="name" required minLength={2} maxLength={80} autoComplete="name" placeholder="Nama kamu"/></label>}<label>Email<input name="email" required type="email" autoComplete="email" placeholder="nama@email.com"/></label><label>Password<input name="password" required type="password" minLength={8} autoComplete={authMode === "login" ? "current-password" : "new-password"} placeholder="Minimal 8 karakter"/></label><button className="primary large" type="submit" disabled={authBusy}>{authBusy ? "Memproses..." : authMode === "login" ? "Masuk →" : "Buat akun →"}</button><button className="switchAuth" type="button" onClick={() => setAuthMode(authMode === "login" ? "register" : "login")}>{authMode === "login" ? "Belum punya akun? Daftar" : "Sudah punya akun? Masuk"}</button></form></div>}
